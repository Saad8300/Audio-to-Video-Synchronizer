...
export function TextOverlayPreview({ settings }: { settings: any }) {
  const {
  ...
  return (
    <div className="card p-4 space-y-3 sticky top-6">
      ...
      <div className="rounded overflow-hidden relative shadow-inner" style={{ aspectRatio: aspectRatio.replace(':', '/'), background: 'linear-gradient(135deg, #1e293b, #0f172a)' }}>
        <div 
          className="absolute"
          style={{
    ...
    <div className="card transition-all duration-300">
      ...
      {isOpen && (
        <div className="px-5 pb-5 pt-0 border-t" style={{ borderColor: 'var(--border-subtle)' }}>
          ...
          <div className="mt-4 flex items-center gap-6">
            ...
            {enabled && (
              <div className="flex items-center gap-2 flex-1">
                <label className="text-[11px] font-semibold text-[var(--text-muted)]">Mode:</label>
                <select 
                  value={mode}
          ...
          <div className="space-y-5 mt-5 transition-opacity" style={{ opacity: enabled ? 1 : 0.4, pointerEvents: enabled ? 'auto' : 'none' }}>
            ...
            <div className="flex items-center gap-3 p-3 rounded" style={{ background: 'var(--bg-elevated)' }}>
              ...
              <button onClick={handleApplyPreset} disabled={!selectedPresetId} className="px-3 py-1 rounded text-xs font-bold text-white bg-[var(--accent-primary)] hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed transition-all">Apply</button>
              <button onClick={handleSavePreset} className="px-3 py-1 rounded text-xs font-bold bg-[var(--bg-input)] hover:bg-[var(--border-subtle)] transition-all text-[var(--text-primary)] border border-[var(--border-subtle)]">Save</button>
            ...
            {mode === 'whole_video' && (
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]">Overlay Text</label>
                <input 
                  type="text" 
                  value={text}
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                <div className="p-3 rounded bg-[var(--bg-input)] border border-[var(--border-subtle)] flex items-center justify-between cursor-pointer" onClick={() => onChange({ textOverlayShadowEnabled: !shadow })}>
                ...
                <div className="p-3 rounded bg-[var(--bg-input)] border border-[var(--border-subtle)] space-y-2">
