...
with open("backend/video_timeline_generator.py", "r") as f:
    content = f.read()

# 1. Update parse_timeline_csv to extract text
csv_search = """        rows.append({
            "idx": i,
            "start": start_sec,
            "end": end_sec,
            "video": video_str,
            "video_path": video_map[video_str]
        })"""

csv_replace = """        text_val = str(r.get("text", "")).strip()
        rows.append({
            "idx": i,
            "start": start_sec,
            "end": end_sec,
...
content = content.replace(csv_search, csv_replace)
...
with open("backend/video_timeline_generator.py", "w") as f:
    f.write(content)
print("video_timeline_generator.py patched")
