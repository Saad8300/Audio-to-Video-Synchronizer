...
# ---- Move to the project root (so it works when double-clicked from Finder) ----
...
# =============================================================================
header "Step 2 — Checking port availability"
...
if check_port 8000; then
    warn "Port 8000 (backend) is already in use!"
    echo "    → Another process is using this port."
    echo "    → Run stop_app.command first, then try again."
    PORT_CONFLICT=1
fi

if check_port 5173; then
    warn "Port 5173 (frontend) is already in use!"
    echo "    → Another process is using this port."
    echo "    → Run stop_app.command first, then try again."
    PORT_CONFLICT=1
fi
...
if [ "$PORT_CONFLICT" -eq 1 ]; then
    echo ""
    error "Port conflict detected. Please run stop_app.command and try again."
    echo -e "\nPress any key to close this window..."
    read -n 1
    exit 1
fi

success "Ports 8000 and 5173 are free"
...
if [ "$WAITED" -ge "$MAX_WAIT" ]; then
    echo ""
    warn "Backend did not start in ${MAX_WAIT}s. Check logs/backend.log for errors."
fi
...
if [ "$WAITED" -ge "$MAX_WAIT" ]; then
    echo ""
    warn "Frontend did not start in ${MAX_WAIT}s. Check logs/frontend.log for errors."
fi
